...
public class CommandLocation extends Command {
	...
	public CommandLocation(String command, String[] args, Launcher launcher, MessageEvent<PircBotX> event) {
		super(command, args, launcher, event);
		if(command.contains("game"))
			MinecraftChatHandler.getInstance().messageQueue.add("The Mini-game that I'm currently in is " + MinecraftCurrentInfo.currentServerRoomUUID);
		else
