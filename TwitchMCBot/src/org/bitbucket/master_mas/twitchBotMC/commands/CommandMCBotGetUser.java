...
public class CommandMCBotGetUser extends Command {

	public CommandMCBotGetUser(String command, String[] args,
			Launcher launcher, MessageEvent<PircBotX> event) {
		super(command, args, launcher, event);
		
		MinecraftChatHandler.getInstance().messageQueue.add("MCBot Currently Using Username - " + launcher.getBot().getConfiguration().getName());
	}
}
